...
describe("text_matches_across", () => {
  const SENTENCE =
    "UK-based and available now. Open to backend roles, hybrid or remote, " +
    "with a start date that can move to suit the team.";
  ...
  it("matches across a line break that splits the text", () => {
    const dir = tempRepo();
    writeFileSync(join(dir, "wrapped.md"), "with a start date that\ncan move to suit the team\n");
    expect(
      checkTextMatchesAcross(dir, {
        text: "with a start date that can move to suit the team",
        files: ["wrapped.md"],
      }).status,
    ).toBe("passing");
  });
  ...
  it("does not match a line break when normalization is turned off", () => {
    ...
    expect(
      checkTextMatchesAcross(dir, {
        text: "with a start date that can move",
        files: ["wrapped.md"],
        normalize: [],
      }).status,
    ).toBe("failing");
